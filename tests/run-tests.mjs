...
console.log('\n複数案の同時打診:');
{
    ...
    test('★ 本人の画面に期限と「ほかの案と同時」を出す (通知を見逃した人に伝わらない)', () => {
        ...
        // ★ 文字列があるだけでは足りない — 組み立てた注記を**実際に描いている**こと
        assert.ok(/\$\{notes\.length \? `<div/.test(fn), '注記を組み立てただけで描いていない');
        assert.ok(/notes\.map\(n => `<div>\$\{n\}<\/div>`\)/.test(fn), '注記の並べ方が変わった');
    });
...
console.log('\nopsStageDomain:');
{
    ...
    test('★ 配線: 運営タブは段階で出し分ける (段階ヘッダ・ヒーロー・チェックリスト・その他・設定タブの運営ブロックの移動)', () => {
        ...
        assert.ok(/renderSettingsNotifyStatus\(\)/.test(etcOpen) && /renderSettingsActivity\(\)/.test(etcOpen) && /renderSettingsCharMaster\(\)/.test(etcOpen), 'その他 を開いたときに運営ブロックを描いていない');
        // 👑 互換ゲート・運営担当・メンバー管理・復元は設定タブの master 区画 (2026-09-12・Codex と合意)。メンテナンスでは描かない
        assert.ok(!/renderClientGateSettings\(\)/.test(etcOpen) && !/renderSettingsRoles\(\)/.test(etcOpen) && !/renderSettingsMemberCount\(\)/.test(etcOpen), 'master の区画をメンテナンスでも描いている');
        assert.ok(/if \(typeof _opsRoleForCurrent === 'function' && _opsRoleForCurrent\(\) === 'master'\) \{\s*tasks\.push\(renderSettingsMemberCount\(\), renderClientGateSettings\(\), renderSettingsRoles\(\)\);/.test(stg), '設定タブが master の区画を役割で描いていない');
...
console.log('\ngrowthDomain:');
{
    ...
    test('★ 配線: 役割で運営画面が決まる (名乗り直し・起動・90秒ごとに読む) / master だけ 🛠 / 任命パネル / SQL 46 ⇄ JS', () => {
        ...
        assert.ok(/<div class="dc-card" data-span="6" data-master-only>\s*<div style="[^"]*">\s*<div style="[^"]*">👑 運営担当<\/div>/.test(html) && /id="settingsRolesBody"/.test(html), '任命パネルが無い / data-ops-only が付いている (🛠 OFF で消える)');
        // 👑 区画は設定タブに残す (メンテナンスへ移さない): 互換ゲート / 運営担当 / メンバー管理 / 復元。バックアップ保存はメンテナンス
        ...
        assert.ok(bk && !/settingsRestoreFile/.test(bk) && /handleSettingsBackup\(\)/.test(bk), 'バックアップ保存のカードに復元が残っている / 保存がメンテナンスから消えた');
        assert.ok(/<div class="ops-group-label" data-master-only[^>]*>👑 マスター運営/.test(html), '👑 区画の見出しが無い');
        const gate = html.match(/async function renderClientGateSettings\(\)[\s\S]*?\n        \}/)?.[0] || '';
        assert.ok(/_opsRoleForCurrent\(\) !== 'master' : !_opsMode\) \{ el\.textContent = ''; return; \}/.test(gate), '互換ゲートの描画が 🛠 で決まったまま (master が 🛠 OFF だと消える)');
