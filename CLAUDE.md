...
- **🧬 育成をくらべる** (D1・2026-09-09)。運営タブ「残り戦闘可能メンバー」の名前をタップ →
  ...
  **両者そろっているいちばん新しい回**を選ぶ — 片方だけ新しい回を取り込んでいると、もう片方が全部「—」になり
  ...
  ★ **読み方が値そのもの** (2026-09-09 モック 6f9d0510 で決定): 結論を先に (何体で上か / 合計と差) →
  **差の大きい順** (`rankSquad`) → **差のある項目だけ**チップに (戦闘力は横棒で出しているのでチップにしない) →
  全9項目とオーバーロードは畳んでおく。開閉は**番号**で持つ (キャラ名を属性に埋めない)。編成を変えたら捨てる。
  ★ 合計と「何体で上か」は**両方の戦闘力がある体だけ**で数える — 片方にしか無い体を混ぜると、
  違う顔ぶれの合計を「差」と呼ぶことになる。
