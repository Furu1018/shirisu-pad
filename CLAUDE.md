...
- **測定ボスレベル (player_damages.boss_level)**: 模擬で測ったレベル L の編成は
  **対象レベル ≤ L にだけ**使う (`usableAtLevel`)。高難度で出せた出力は低難度なら確実に出せるが
  逆は下がりうるため。**`level == null` = 未指定 = 全レベルで使える** (移行互換 — ここを Lv1 に
  倒すと既存の提出が Lv2 以降で使えなくなりプランが崩壊する)。**Lv4 (ボス5・HP無限) は最上位**なので
  Lv4 測定かレベル未指定でないと出せない。絞り込みは候補列挙・必須枠の予約 (`canUseAttr`)・
  Lv4割当・温存の機会費用 (`oppCostOf`)・締め凸候補 (`renderOpsFinishList`) の**全部**に効かせること —
  片方だけだと「候補にならないのに枠だけ予約される」等の不整合になる
