...
- **🧬 育成くらべ** (分析タブの「育成」ビュー・2026-09-09 に作り直し)。レイドが終わったあとに
  ...
  **差の大きい順** (`rankSquad`) → **差のある項目だけ**チップに (戦闘力は横棒で出すのでチップにしない) →
  ...
  ⚠ アプリ内にバーストの色が**2組ある** — 編成エディタ `TE_BURST_COLOR` (B1緑/B2黄/B3赤/BΛ紫) と
  キャラ管理 `_BURST_COLOR` (B1紫/B2緑/B3赤/BΛ紫)。育成くらべは編成を組む文脈なので前者に合わせた。
  **どちらかに寄せる判断は保留** (キャラ管理の見た目が変わるためユーザー確認待ち)。
