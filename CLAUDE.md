...
**標準フロー (2026-07-25 更新・ユーザー指示 / 2026-09-11 に投げ方を更新)**: 実装 → commit → **同一ターン内で
`codex exec` を Bash から直接呼んで監査** (下の「Codex 監査の投げ方」。codex-rescue エージェントは使わない) → 指摘対応 → **同一ターンで push まで完了**。
...
- **js/domain/raidReview.js** — 分析タブ › **🛡 全体分析** (レイドの振り返り・2026-09-15 ユーザー要望・Codex と設計を合意)。
  ★ **実績 (tab-ranking) = 個人の成績 / 全体分析 = ユニオン全体** と分ける。セグメントは 📊実績 の**隣** (`_setSlvView('review')` / `_gotoSlvView('review')`)。
  先頭に「この画面で分かること」の説明カード (★ 進捗率は強さではない、を明記)。カード3枚:
  ① **属性別の進捗** (ボス×Lv の積み上げ・どこまで削れたか) ② **得意・不得意** (散布図 凸数×強さ / 横棒ランキング・切り替えは端末に記憶
  `shirisuko_rr_apt_v1`) ③ **誰がどの属性で** (人×属性のヒートマップ)。
  ★ **進捗率は達成度であって火力ではない** — 踏破すると 100% で頭打ちになり、かけた凸数で決まる。だから強さは別カードにし、
  主役の指標は **1凸あたりの対GB中央値比** (SLv補正・締め凸を除く)。GB が無い月は「SLv を揃えた 1凸平均」、SLv 表も無ければ記録のまま (basis で出し分け)。
  ★ **ボスのクラス (tyrant/lord) は推定**: raid-config の `bossClassByMonth` が無い月は「**踏破した Lv の合計が設定HPと厳密に一致**」から決める
  (ゲームは残HPを超えたダメージを記録しないので、踏破した Lv は合計が HP と一致する)。**一意に決まらなければ率を出さない** (母数をでっち上げない)。
  ...
  ★ **幅は `raidReviewDomain.sliceWidths(L)` が決める** — 細い凸に下限を与えつつ**合計は必ず「その Lv で削った割合」に一致**させる
  (CSS の `min-width: px` だと本数が多いとき合計が親を超えて右端の凸が切れる: Codex指摘)。凸のボタンは `data-attr/lv/i` を持ち、
  ✕ も同じ凸を指す (差し替えのあと押した凸へ `focus({preventScroll:true})` を戻す)
...
- **通知の配管** (監査 2026-09-12 A1〜A4)。★ send-push の**時間帯フィルタは「戦闘可能時間」で絞る**ので、本人あて・行動が要る通知は `ignoreAvailability: true` を必ず付ける
  (締め凸依頼・見送り・📣 お願い・予約の解除/承認/却下・催促・凸報告/模擬提出のお願い)。撃破 (交戦中の人あて)・Lv開放・戦闘可能時間・配信の更新・一斉送信は
  以前から無視している。**いまフィルタを通るのは「強行凸の警告」だけ** = 時間帯フィルタは事実上「送り手が付け忘れたときの罠」なので、新しい push には必ず付けること。
...
node tests/raid-review.mjs    # レイドの振り返り (属性別の進捗 / 得意・不得意 / 誰が) の実行テスト — 本番の 2026-09 のデータで描く
```
...
- ★ **トークンを使いすぎない** (2026-09-14 に 1 日で 1.4M 使い、利用上限で監査が止まった)。モデルは
  `~/.codex/config.toml` の **gpt-5.6-terra / high のまま** (ユーザー決定: 質を優先して High は下げない。**別 PC の config.toml も同じにする**)。
  下げるのは**依頼の出し方**:
  - **範囲を絞る** — `git diff A..B` と関係する関数名・行番号を列挙する。「システム全体を見て」は 1 回で 400k 使う
    (Codex は shell でファイルを開きながら読むので、27k 行の index.html を何度も読む)
  - **小さい修正は diff を依頼文に貼る** (Codex にファイルを開かせない → 95k で済んだ例あり)
  - **並列に 3 本投げない** (初回の棚卸しだけの手)。**修正はまとめてから 1 回**監査する (1 コミットごとに投げない)
  - 上限に当たったら `ERROR: You've hit your usage limit ... try again at HH:MM` と返る。**Monitor で時刻まで待って自動実行**できる
    (`until [ "$(date +%H%M)" -ge HHMM ]; do sleep 60; done; codex exec ...`、persistent: true)
- ★ 全体の棚卸し (数週間に 1 回) は `docs/codex-audit-prompts-2026-09-14.md` の依頼文 3 本 (凸枠 / 役割・通知 / 全体の健全性) を
  そのまま使い回す (作り直さない。各 200〜400k トークンなので 1 本ずつ)。修正後の再監査の型も同じファイルにある
- ★ **Codex の指摘は、既存テストと CLAUDE.md の設計判断に突き合わせてから直す**。2026-09-14 に 2 件の誤指摘
  (任命パネルの場所 / 催促 Push の時間帯フィルタ) をそのまま信じかけ、後者は既存テストが落として気づいた
