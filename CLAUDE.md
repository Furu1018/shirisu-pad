...
- **ボス横断の限定分岐 (フェーズ2)**: 貪欲は「そのボスで最良」を選ぶので、別ボスでしか
  ...
  (幅16・深さ3 = 基準解を含め最大49シナリオ。`scenarioBudgetFor` の60は40人超の異常入力への保険で、
  30人ユニオンでは実際の制約は幅・深さ側)。**実時間で打ち切らない** — 配信は「押すたびに同じ
...
- **効果測定は `node tests/bench-crossboss.mjs`** (seed固定の乱数2000盤面)。
  ...
  盤面は本番前提 (レベル別HP定数・B1,2,4=lord/B3,5=tyrant・`loadoutsByAttr` で
  汎用B1/B2を共有させたキャラ被り) に合わせてある。**`damagesByAttr` だけの盤面で測ると
  slot/ord・同属性2凸・キャラ被りが評価されず、数字が実態より良く出る** (2026-08-03 に実際にやった)
