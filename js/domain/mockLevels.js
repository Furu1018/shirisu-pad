// ============================================================================
// ドメイン: 模擬スロットの測定値 (levels JSONB — 31_player_damages_levels.sql)
// ----------------------------------------------------------------------------
// 「1スロット = 1編成」。スロットの中の levels はレベル別の測定値だったが、
// **測定レベルの概念は 2026-09-06 に廃止した (課題A)**。
//   levels = { "0": 14.2, "4": 12.5 }   キー: "0"=レベル未指定〜"4" / 値: B単位ダメージ (正の有限数)
//
// ★ いまの使用ルール: **1編成 = ダメージ1つ。どのレベルにも使える**。
//   複数キーを持つのは廃止前の既存データだけで、読むときは `representativeDamage` で
//   **中央値**に畳む (最大値は試行のブレの上振れを固定する)。新規保存は単一値。
//   既存の levels は書き換えず読み取りだけで畳んでいる (移行の巻き戻し余地を残すため)。
//   旧ルール「記録レベル L の測定は対象レベル ≤ L でのみ使える」は**復活させないこと** —
//   実データが想定を否定している (同一編成のレベル違い6件すべて Lv1比 96〜105%)。
//   `bestAtLevel` は @deprecated。バックアップ復元など廃止前データをそのまま読む箇所のためだけに残す。
//
// 不変条件 (書き込み側 = supabase-client.js の _upsertPlayerDamages が唯一の維持点):
//   damage_b   = levels の最大値 (廃止前の互換ミラー。**表示・計算で直接読まないこと**)
